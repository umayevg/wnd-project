...
<template>
  <div class="bg-[#292929] text-white">
    <div class="flex container mx-auto h-12 mb-12 sticky items-center justify-end">
      ...
      <router-link active-class="active" class=" mr-4 hover:text-indigo-500 transition" to="/search">Search
      </router-link>
      <div class="mr-4 transition cursor-pointer" @click="openModal">
        ...
        <span class="bg-green-600 top-[4px] right-[2px] text-sm w-5 h-5 text-center rounded-full absolute">{{
            store.totalItems
          }}</span>
      </div>
      <CartModal :storeData="store" :isOpen="isModalOpened" @modal-close="closeModal" name="first-modal"/>
    </div>
  </div>
</template>
