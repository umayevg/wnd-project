...
<template>
  <div class="search-container mt-4 mb-4">
    <select v-model="selectedCategory" class="category-select w-[100px] md:w-max lg:w-max outline-0">
      <option value="">Select category</option>
      <option v-for="category of categories" :value="category.name">{{ category.label }}</option>
    </select>
    <input v-model="searchQuery" class="search-input" placeholder="Enter your search" type="text">
